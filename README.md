...
### 多模型提供商与自动回退
- 新增 `model_providers` 配置列表，可分别填写多个中转站或官方接口的模式、地址、Key 和模型
...
| `model_providers` | 多提供商有序回退列表；每项可单独配置接口模式、地址、Key、默认模型和文生图模型 |
...
1. 在管理面板找到 `model_providers`，按“主站 → 备用站 → 最后兜底”的顺序添加条目。
2. 每个条目分别填写接口模式、基础地址、API Key、默认模型；如文生图模型不同，再填写文生图模型。
3. 临时停用某一项时关闭该条目的“启用此提供商”，无需删除配置。
4. `model_providers` 留空时，插件继续读取原有的 `interface_mode`、`base_url`、`api_keys` 和 `model`。
5. 使用 `#提供商列表` 查看序号，再用 `#切换提供商 2` 或 `#切换提供商 提供商名称` 临时更换调用起点。
6. 提供商名称重复时请按序号切换；插件会保存该条目的原始面板序号以避免歧义。
7. 被选中的提供商失败后仍会继续尝试其他启用项；使用 `#切换提供商 自动` 恢复面板顺序。
