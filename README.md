...
### 多模型提供商与自动回退
- 面板上方的 `interface_mode`、`base_url`、`api_keys` 和默认模型自动作为第一优先级主提供商
- 下方 `model_providers` 配置列表作为备用提供商，可分别填写多个中转站或官方接口的模式、地址、Key 和模型
- `#提供商列表` 会统一显示上方主提供商和下方备用提供商
- 严格按面板中的列表顺序调用；当前提供商报错、抛出异常或返回空结果时，自动尝试下一项
- 缺少接口地址或 API Key 的条目会直接跳过，不发起无意义请求
- 任意提供商成功返回图片后立即停止，不会继续消耗后续提供商
- 每个提供商独立复用 HTTP 会话和 Key 轮询状态；切换起点不会重建其他提供商连接
- 列表留空时继续使用原有单提供商配置，方便平滑迁移
...
| `#提供商列表` | `供应商列表` | 查看多提供商状态、模型和当前调用起点（管理员） |
| `#切换提供商 <序号或名称>` | `切换供应商` | 从指定提供商开始调用，失败后继续回退到其余启用项（管理员） |
| `#切换提供商 自动` | - | 恢复管理面板中的默认提供商顺序（管理员） |
...
| 配置项 | 说明 |
| --- | --- |
| `interface_mode` | 上方主提供商的接口模式：`openai_image` 标准图片接口；`openai_chat` 对话中转；`gemini_official` Gemini 官方；`custom_endpoint` 自定义完整路径 |
| `base_url` | 上方主提供商的统一基础地址；OpenAI/Gemini 模式会自动补全路径 |
| `api_keys` | 上方主提供商的 Key 池，支持按行填写多个 Key |
| `model_providers` | 下方备用提供商列表；主提供商失败后按这里的顺序回退 |
| `active_provider` | 当前提供商调用起点；建议通过 `#切换提供商` 管理，留空表示自动按面板顺序 |
| `model_list` | 可用模型 ID 列表，默认包含 nano-banana 等 |
| `model` | 上方主提供商的默认模型，可直接填写任意模型名称 |
| `text_to_image_model` | 上方主提供商的文生图专用模型，留空时使用默认模型 |
...
1. 先在面板上方填写主提供商的接口模式、地址、API Key 和默认模型。
2. 在下方 `model_providers` 中按“备用站 → 最后兜底”的顺序添加条目。
3. 每个备用条目分别填写接口模式、基础地址、API Key、默认模型；如文生图模型不同，再填写文生图模型。
4. 临时停用某一备用项时关闭该条目的“启用此提供商”，无需删除配置。
5. 上方地址和 Key 都留空时，不会加入主提供商；下方列表仍可独立使用。
6. 使用 `#提供商列表` 查看统一序号，再用 `#切换提供商 1`、`#切换提供商 2` 或提供商名称临时更换调用起点。
7. 提供商名称重复时请按统一列表序号切换。
8. 被选中的提供商失败后仍会继续尝试其他启用项；使用 `#切换提供商 自动` 恢复“上方主提供商 → 下方备用列表”的默认顺序。

> `model_providers` 使用 AstrBot 的 `template_list` 配置类型，需要 AstrBot v4.10.4 或更高版本。
